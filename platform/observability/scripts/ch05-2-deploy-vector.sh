...
kubectl -n "$NAMESPACE" get deploy/openobserve >/dev/null 2>&1 || die "OpenObserve is required before Vector logging. Run 05.1 first or deploy OpenObserve before Vector."
log "Applying Vector manifests"
kubectl apply -f "$REPO_ROOT/manifests/vector/vector.yaml"
kubectl -n "$NAMESPACE" rollout status ds/vector --timeout=300s || true
log "Vector deployment complete"

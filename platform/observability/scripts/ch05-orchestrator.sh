...
deploy_vm_stack() {
  ...
  # CH05 owns the Grafana Helm release. After CH05.5 enables SSO, reconcile mode
  ...
  if [[ "${DEPLOY_MODE}" == "baseline" ]] && kubectl -n "${NAMESPACE}" get secret grafana-authentik-oauth >/dev/null 2>&1; then
    log "Grafana SSO secret detected; baseline mode may reset Grafana OAuth Helm values. Run 05.5 after CH05 baseline if SSO disappears."
  fi

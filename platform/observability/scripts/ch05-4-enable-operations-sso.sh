...
start_authentik_api_port_forward(){
  ...
  kubectl -n "${IDENTITY_NAMESPACE}" port-forward --address 127.0.0.1 svc/authentik-server "${AUTHENTIK_LOCAL_PORT}:80" >/tmp/ch05-4-authentik-port-forward.log 2>&1 &
...
start_zabbix_api_port_forward(){
  ...
  kubectl -n "${NAMESPACE}" port-forward --address 127.0.0.1 svc/zabbix-web "${ZABBIX_LOCAL_PORT}:8080" >/tmp/ch05-4-zabbix-port-forward.log 2>&1 &
  ...
  cat /tmp/ch05-4-zabbix-port-forward.log >&2 || true
...
configure_openobserve_sso_secret(){
  log "Applying OpenObserve Enterprise native SSO secret"
  ...
  kubectl -n "$NAMESPACE" rollout restart deploy/openobserve >/dev/null
  kubectl -n "$NAMESPACE" rollout status deploy/openobserve --timeout=300s
}
patch_zabbix_web_for_saml_runtime(){
  log "Patching Zabbix frontend runtime SAML settings"
  ...
  kubectl -n "$NAMESPACE" patch deploy zabbix-web --type=strategic -p '{"spec":{"template":{"spec":{"volumes":[{"name":"zabbix-saml-certs","secret":{"secretName":"zabbix-saml-certs","optional":false}}],"containers":[{"name":"zabbix-web","volumeMounts":[{"name":"zabbix-saml-certs","mountPath":"/usr/share/zabbix/conf/certs","readOnly":true}]}]}}}}' >/dev/null
  kubectl -n "$NAMESPACE" set env deploy/zabbix-web ZBX_SSO_IDP_CERT=/usr/share/zabbix/conf/certs/idp.crt ZBX_SSO_SETTINGS="{\"strict\":false,\"baseurl\":\"https://zabbix.${BASE_DOMAIN}\",\"use_proxy_headers\":true}" >/dev/null
...
ensure_tls_issuer_state
WORKDIR="$(mktemp -d)"
render_tpl "$REPO_ROOT/manifests/sso/operations-native-sso-ingress.yaml.tpl" "$WORKDIR/operations-native-sso-ingress.yaml"
log "Applying native-SSO operations ingresses"
kubectl apply -f "$WORKDIR/operations-native-sso-ingress.yaml"
log "Operations native SSO applied"

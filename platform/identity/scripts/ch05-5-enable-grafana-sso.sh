...
validate_prerequisites() {
  kubectl get ns "${IDENTITY_NAMESPACE}" >/dev/null 2>&1 || die "Missing namespace: ${IDENTITY_NAMESPACE}; deploy CH04.5 first"
...
resolve_authentik_api_token() {
  ...
  [[ -n "${AUTHENTIK_BOOTSTRAP_TOKEN}" ]] || die "Missing AUTHENTIK_BOOTSTRAP_TOKEN; re-run CH04.5 to create/preserve authentik-bootstrap secret"
...
render_values_overlay() {
  local tmp_values="/tmp/ch05-5-grafana-sso-values.yaml"
  sed \
